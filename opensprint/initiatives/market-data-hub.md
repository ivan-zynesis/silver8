...
- [ ] **gateway-ws**: WebSocket server with subscribe/unsubscribe op protocol (DEC-012). Per-consumer ring buffer with drop-oldest backpressure + sustained-overflow disconnect (DEC-011); serialize-once-fan-out-many. Rebalance hint message on SIGTERM (DEC-019). Tests: churn (10k connect/disconnect), slow consumer (queue bounded + disconnected, others unaffected), subscribe/unsubscribe storm (no orphan registry entries).

- [ ] **mcp-server**: NestJS module wrapping `@modelcontextprotocol/sdk`. Tool surface (DEC-015): `list_topics`, `describe_topic`, `get_top_of_book`, `get_book_snapshot`, `get_hub_status`. Zod schemas for all args. Resources at `market://<venue>/book/<symbol>` with `resources/subscribe` and `notifications/resources/updated` (DEC-013). Dual transport: HTTP+SSE primary, stdio behind `MCP_TRANSPORT=stdio` flag (DEC-014). MCP `notifications/rebalance` parity with WS rebalance event. `get_hub_status` returns same payload as HTTP `/status` (DEC-022).

- [ ] **docs-ship-package**: `/docs` written for LLM consumption per DEC-023 structure (overview, getting-started, MCP tool reference, MCP resources, topics, worked examples, failure modes, WS gateway, architecture write-up). Validate by handing draft docs to a fresh LLM agent and running the worked examples; iterate on stumbles. Multi-stage Dockerfile (distroless final). `docker-compose.yml` for local dev. README. One-page architecture write-up covering scale path (DEC-017), deployment variants (DEC-016), production target (DEC-024), what's-lost-on-restart. Record 10-minute walkthrough.
